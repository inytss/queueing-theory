...
* mengetahui sistem antrian
* mengetahui asumsi pada teori antrian
* mengetahui disiplin antrian
* mengetahui kondisi yang harus ada pada sistem antrian
* mengetahui model antrian yang digunakan dari suatu sistem antrian
* mengukur kinerja sistem antrian
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Dalam sistem antrian ada notasi yang merangkum semua karakteristik untuk menggambarkan model antrian. Ini adalah pengaturan notasi Kendall:
...
<center> $L_s = L_q + \frac{\lambda}{\lambda} $
...
3. Single channel-multi phase
   ...
   Sistem antrin ini merupakan tipe atau model antrian yang memiliki satu jalur antrian dan beberapak fase pelayanan yang disusun secara seri. Desain pelayanan seperti ini biasanya diterapkan pada pelayanan perpanjangan surat ijin mengemudi (SIM) dan pelayanan pembuatan pasport. Biasanya pada struktur antrian jenis ini terdapat beberapa loket yang tersusun secara seri yang harus dilewati oleh pelanggan hingga pelayanannya selesai. 
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Kita akan coba untuk analisis sistem antrian yanng ada pada hari tertentu. Untuk melakukan analisis antrian, kita perlu mengetahui ukuran-ukuran kinerja yang terjadi pada sistem antrian service tersebut. Oleh karena itu kita akan menggunakan bantuan package `queuecomputer` untuk analisis lebih lanjut dari sistem antrian yang kita miliki.

### Load package dan data
```{r}
# read data
service <- read.csv("data_input/service.csv")

# service %>% 
#    # rename(arrival = service, service = arrival) %>% 
#    # write.csv("data_input/service.csv", row.names = F)
#    filter()
head(service)
```

Penjelasan untuk data `service` adalah sebagai berikut:
...
fit <- fitdistr(service$arrival, "exponential")
fit2 <- fitdistr(service$service, "exponential")

# goodness of fit test
ks.test(service$arrival, "pexp", fit$estimate)
ks.test(service$service, "pexp", fit2$estimate)
```
...
```{r}
source("steady_state.R")
steady_state(service)
```
...
# define input 
c <- 13
lambda <- mean(service$arrival)
mu <- mean(service$service)
input_mm1 <- queueing::NewInput.MM1(lambda = 1/lambda, mu = 1/mu)
input_mmc <- queueing::NewInput.MMC(lambda = 1/lambda, mu = 1/mu, c = c)
...
que <- queue_step(arrivals = service %>% pull(arrival), 
                  service = service %>% pull(service), servers = 13)
summary(que)[10]
```
...
1. Tingkat kesibukan sistem antrian yang ada pada service sepeda motor masih belum baik dilihat dari nilai faktor utilisasi sebesar 0.79 yang artinya 79% dari waktu pelayanan yang ada fasilitas server pada service sepeda motor cukup sibuk.
2. Rata-rata waktu yang dihabiskan satu pelanggan dalam antrian yaitu sebesar 5.5 jam jika menggunakan satu fasilitas pelayanan, namun jika menggunakan 13 fasilitas pelayanan waktu yang dihabiskan untuk menunggu yaitu sebesar 0.07 jam atau 4.2 menit.
3. Rata-rata waktu yang dihabiskan satu pelanggan dalam sistem saat melakukan service sepeda motor yaitu sebesar 5.5 jam jika menggunakan satu fasilitas pelayanan, namun jika menggunakan 13 fasilitas pelayanan waktu yang dihabiskan dalam sistem antrian yaitu sebesar 0.87 jam atau 52.2 menit.
